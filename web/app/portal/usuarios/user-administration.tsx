...
const roles = ["ADMIN", "MANAGER", "FINANCE", "PHARMACIST", "OPERATOR", "VIEWER"];
const labels: Record<string, string> = {
  OWNER: "Proprietário", ADMIN: "Administrador", MANAGER: "Gestor", FINANCE: "Financeiro",
  PHARMACIST: "Farmacêutico", OPERATOR: "Operador", VIEWER: "Consulta",
};
...
export function UserAdministration({ users, invitations, currentRole }: { users: UserEntry[]; invitations: PendingInvitation[]; currentRole: string }) {
  ...
  async function invite(formData: FormData) {
    ...
    const body = await response.json().catch(() => ({})) as { message?: string; inviteUrl?: string };
    if (!response.ok) setError(body.message ?? "Não foi possível criar o convite.");
    else { setInviteUrl(body.inviteUrl ?? ""); router.refresh(); }
    setBusy(false);
  }
