...
import { CatalogImportPanel, MatrixImportPanel, MatrixReviewActions, ReleaseReviewActions } from "./catalog-actions";
...
export default async function FiscalCatalogsPage() {
  ...
  return <section className="report-page fiscal-governance-page">
    ...
    <div className="fiscal-import-actions"><CatalogImportPanel/><MatrixImportPanel/></div>
    <div className="report-layout two-columns">
      ...
      <article className="report-panel fiscal-review-panel"><div className="panel-title"><div><span>CATÁLOGOS</span><h2>Versões em revisão</h2></div><strong>{reviewReleases.length}</strong></div>{reviewReleases.length ? <ReleaseReviewActions releases={reviewReleases}/> : <EmptyReport text="Nenhuma versão oficial aguarda revisão."/>}</article>
      <article className="report-panel fiscal-review-panel"><div className="panel-title"><div><span>MATRIZ DF</span><h2>Regras em homologação</h2></div><strong>{reviewRules.length}</strong></div>{reviewRules.length ? <MatrixReviewActions rules={reviewRules}/> : <EmptyReport text="Nenhuma regra do DF aguarda homologação."/>}</article>
    </div>
  </section>;
}
