...
export type SecuritySession = { id: string; userAgent: string | null; ipAddress: string | null; expiresAt: string; lastSeenAt: string; rotatedAt: string | null; revokedAt: string | null; revokedReason: string | null; createdAt: string; assuranceLevel: number; mfaVerifiedAt: string | null; status: "ACTIVE" | "REVOKED" | "EXPIRED"; user: { name: string; email: string; systemRole: string } };
...
export type SecurityReport = { generatedAt: string; indicators: { activeSessions: number; failedLogins: number; refreshReuse: number; revokedSessions: number; privilegedUsers: number; privilegedMfa: number; mfaCoverage: number; mfaFailures: number }; sessions: SecuritySession[]; events: SecurityEvent[] };
...
const actionLabels: Record<string, string> = { AUTH_LOGIN_SUCCEEDED: "Login autorizado", AUTH_LOGIN_FAILED: "Login recusado", AUTH_REFRESH_ROTATED: "Sessão renovada", AUTH_REFRESH_REUSE_DETECTED: "Reutilização de token bloqueada", AUTH_REFRESH_FAILED: "Renovação recusada", AUTH_SESSION_REVOKED: "Sessão revogada", AUTH_SESSION_LIMIT_REVOKED: "Limite de sessões aplicado", AUTH_TENANT_ACCESS_DENIED: "Acesso à empresa bloqueado", AUTH_MFA_CHALLENGE_CREATED: "Desafio MFA iniciado", AUTH_MFA_SUCCEEDED: "MFA confirmado", AUTH_MFA_FAILED: "MFA recusado", AUTH_MFA_ENROLLED: "MFA ativado", AUTH_MFA_DISABLED: "MFA desativado", AUTH_MFA_STEP_UP: "Identidade reconfirmada" };
