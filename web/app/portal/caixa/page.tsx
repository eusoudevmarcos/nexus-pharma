...
import { CashRegister, type CashCounterOrder, type CashSessionDetail, type CashSessionSummary, type CashStore, type PosProduct, type SaleContext } from "./cash-register";
...
export default async function CashRegisterPage() {
  ...
  const [stores, products, history, discountPolicy, saleContext, counterOrders] = await Promise.all([
    ...
    portalFetch<CashCounterOrder[]>("/api/v1/balcao/fila-caixa"),
  ]);
  ...
  return <section className="report-page cash-page">
    ...
    <CashRegister counterOrders={counterOrders ?? []} currentUserId={session.profile.id} discountLimit={discountPolicy?.maxPercent ?? 0} history={history ?? []} initialSession={initialSession} products={(products ?? []).filter((product) => product.active)} role={session.membership.role} saleContext={saleContext ?? { sellers: [], pharmacists: [] }} stores={stores ?? []}/>
  </section>;
}
