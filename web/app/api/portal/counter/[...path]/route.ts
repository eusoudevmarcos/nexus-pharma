...
const target = async (context: Context) => `/api/v1/balcao/${(await context.params).path.join("/")}`;
...
export async function PATCH(request: NextRequest, context: Context) {
  return proxyPortal(await target(context), { method: "PATCH", body: await request.text() });
}
